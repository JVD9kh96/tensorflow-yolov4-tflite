...
class conv_prod_v2(tf.keras.layers.Layer):
    def __init__(self, filter_size=(2,2), strides=(2,2), standardized=False, preserve_depth=False, upsample=False, trainable_sum=False):
        ...
        self.preserve_depth = preserve_depth
        self.upsample       = upsample
        self.trainable_sum  = trainable_sum
    ...
    def call(self, feature_map_1, feature_map_2, feature_map_3, training=False):
        ...
        kernel_3 = patch_extractor((self.filter_size[0], self.filter_size[1]))(feature_map_3)
        kernel_3 = tf.transpose(kernel_3, perm=[0, 3, 1, 2])
        ...
        kshape_3 = feature_map_1.shape.as_list()
        MB       = shape[0]
        ...
        kernel_2 = tf.transpose(kernel_2, perm=[0, 2, 1, 3, 4, 5])
        
        kernel_3 = tf.reshape(kernel_3, [kshape_3[0], 
                                     self.filter_size[0],
                                     self.filter_size[0], 
                                     kshape_3[3],
        ...
        qkT = tf.reshape(tf.reduce_sum(tf.nn.softmax(kernel_1 * kernel_2 * self.w, axis=1) * kernel_3 + self.b, axis=[1, 2, 5]), (kshape_1[0],
