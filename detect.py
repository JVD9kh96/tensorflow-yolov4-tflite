...
flags.DEFINE_integer('size', 704, 'resize images to')
flags.DEFINE_string('image', './data/kite.jpg', 'path to input image')
flags.DEFINE_string('output', "./data/detection/", 'write image path')
...
def detect(weights = "./Model/ModelWeights",
             INPUT_SIZE = 416,
             predicted_dir_path = './predicted',
             images_path='kite.jpg',
             model_name = 'yolov4',
             ...
             XYSCALE = None):
    """
    weights           :  path to the saved model weights, string
    INPUT_SIZE        : the input size of the model input, int
    predicted_dir_path: Where to save the results. It should be a directory path, string
    images_path       : Either path to a single image or path to a txt which each line contains a path to an image, string
    is_tiny           : Whether the model is tiny or noy, boolean
    """
    ...
    for num, line in enumerate(input_file):
        ...
        cv2.imwrite(os.path.join(predicted_dir_path, image_name), image)
...
def main(_argv):
    ...
    detect(weights = FLAGS.weights,
             ...
             images_path=FLAGS.image,
             predicted_dir_path = FLAGS.output,
